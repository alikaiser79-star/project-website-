...
export interface NowResult { items: NowItem[]; calm: string | null; }

export function buildNow(now = Date.now()): NowResult {
  const items: NowItem[] = [];
  const lanes = new Set<string>();
  const add = (lane: string, it: NowItem) => { if (lanes.has(lane) || items.length >= 3) return; lanes.add(lane); items.push(it); };
  ...
  if (items.length) return { items: items.slice(0, 3), calm: null };

  /* nothing needs him — one honest, grounding line. */
  return { items: [], calm: calmLine(now) };
}
