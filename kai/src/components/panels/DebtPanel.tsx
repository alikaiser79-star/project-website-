...
export default function DebtPanel({ delay = 0 }: { delay?: number }) {
  ...
  const util = Math.min(100, Math.max(0, (current / debt.limit) * 100));
  const available = Math.max(0, debt.limit - current);
  const animPct = useCounter(util, { decimals: 1, duration: 1.6 });
  const animAvail = useCounter(available, { duration: 1.6 });

  const r = 70;
  const C = 2 * Math.PI * r;
  const dash = (animPct / 100) * C;
  /* Utilisation ring runs hotter as it fills: gold under 50%,
     amber-to-red above. */
          ...
          <svg viewBox="0 0 180 180" width="160" height="160">
            <defs>
              <linearGradient id="dg" x1="0" x2="1" y1="0" y2="1">
                ...
                <stop offset="100%" stopColor="#FF7A48" />
              </linearGradient>
            </defs>
            ...
            />
            <text x="90" y="86" textAnchor="middle" fill={ringColor} fontFamily="Inter, system-ui, sans-serif" fontSize="30" fontWeight="200" letterSpacing="-1">
              {animPct.toFixed(0)}%
            </text>
        ...
        <div className="flex-1 w-full space-y-4">
          <div>
            ...
            <div className="font-sans text-bone text-2xl font-extralight tabular-nums mt-1">
              {fmt(current)} <span className="font-mono text-steel/70 text-xs">of {fmt(debt.limit)} EGP</span>
            </div>
          </div>
          <div>
            <div className="font-mono text-[10px] tracking-[0.18em] text-steel/65 uppercase">Available</div>
            <div className="font-sans text-bone/85 text-lg font-light tabular-nums mt-1">
              {fmt(animAvail)} <span className="font-mono text-steel/70 text-xs">EGP</span>
            </div>
          </div>
          ...
          <div className="font-mono text-[10px] text-steel/55 leading-relaxed">
            limit {fmt(debt.limit)} EGP · APR {debt.apr}%
          </div>
        </div>
      </div>
    </Panel>
