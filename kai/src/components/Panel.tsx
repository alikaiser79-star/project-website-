...
export default function Panel({ num, title, tag, delay = 0, children, className = '' }: Props) {
  return (
    <motion.section
      ...
      className={'glass rounded-md p-4 flex flex-col min-h-0 ' + className}
    >
      <header className="flex items-center gap-3 pb-3 mb-3 border-b border-amber/10">
        <span className="font-mono text-[10px] tracking-[0.18em] text-amber border border-amber/40 px-1.5 py-0.5">{num}</span>
        <h3 className="font-sans text-bone text-[15px] font-medium tracking-wide flex-1">{title}</h3>
        {tag && (
